...
async def subscribeToOrderbook(websocket, symbol:str, group:str, depth:int, interval:str):
    subscribeMsg = {
        "jsonrpc": "2.0",
        "id": 1,
        ...
        "params": {
            ...
            "channels": [f"book.{symbol}.{group}.{depth}.{interval}"]
        }
    }
    ...
    while (1):
        ...
        print(f"Order Book Update For InstrumentName {symbol}: {response}")
...
async def main():
    ...
    while (1):
        orderbookTask = asyncio.create_task(subscribeToOrderbook(socket, "BTC-15MAR24-90000-P", "10", 10, "100ms"))

orderbookTask = asyncio.create_task(subscribeToOrderbook(websockets.connect("wss.//www.deribit.com/ws/api/v2"), "BTC-15MAR24-90000-P", "10", 10, "100ms"))
